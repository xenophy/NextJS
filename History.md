...
# 0.8.7
======================
  * Add     依存関係にgeoip-liteが追加されました。
  * Add     DirectActionで利用するモジュールメソッド名の先頭に_を付加するとREMOTING_APIの設定に反映されないようになりました。
  * Fixed   ExtDirectによるリクエストで、配列型（クライアント側の同時リクエスト）が処理できなかったバグを修正
  * Change  MySQLのコネクションプーリング廃止、非同期処理時にハングアップするため。
  * Add     DirectActionで利用するモジュールメソッド名の先頭に$を付加するとREMOTING_APIの設定でformHandlerが指定できるようになりました。
  * Add     モジュール内メソッドで、req、res、session、cookieオブジェクトにアクセスできるようになりました。
  * Add     テンプレートでrelpath(トップまでの相対パス)が利用できるようになりました。
  * Add     NX.DirectActionにnamespaceの指定ができるようになりました。
...
# 0.8.4
======================
  * Fixed   worker未使用時、node 0.5.10で削除されたprocess.binding('net')の影響を受けないように修正しました。
  * Add     FileStoreの保存先が/tmp固定だったのを設定できるようになりました。
  * Add     セッション保存先を指定できるようになりました。
