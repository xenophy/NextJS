...
NX.module.adapter.MongoDB = NX.extend(NX.module.adapter.AbstractAdapter, {
    ...
    createCollection : function(name, opt, cb) {
        this.db.createCollection(name, opt, function(err) {
            cb();
        });
    },
    ...
    listDatabases : function(cb) {
        ...
        var me = this;
        var db = new mongo.Db('admin', me.server, {});

        db.open(function(err) {
            db.executeDbCommand({'listDatabases':1}, function(err, result) {
                var dbs = result.documents[0].databases;
                db.close();
                cb(dbs);
            });
        });
    ...
    renameCollection : function(src, dest, cb) {
        this.db.renameCollection(src, dest, function(err) {
            cb();
        });
    },
