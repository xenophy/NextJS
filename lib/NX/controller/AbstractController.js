...
NX.controller.AbstractController = NX.extend(require('events').EventEmitter, {
    ...
    execute : function() {
        ...
        var oraCls = getOverrideActionClass(me, me.actionName);
        ...
        if(oraCls && !NX.isArray(oraCls)) {
            ...
            oraCls.forEach(function(cls, i) {
                ...
                } else if(NX.isFunction(cls)) {

                    // NX.Actionクラス化
                    cls = convertToActionClass(me, cls);
            ...
            if(actionList.length > ++num) {
                ...
                actionList.forEach(function(action, i) {
                    ...
                    NX.iterate(action.__modules, function(name, mod) {
                        if(mod.connection.connected) {
                            modConn[i][name] = false;
                        }
                    });

                    if(Object.keys(modConn[i]).length === 0) {
                        delete modConn[i];
                    }
                });
                ...
                var f = function(i, name) {
                    ...
                    NX.iterate(modConn, function(key, item) {
                        NX.iterate(item, function(iname, t) {
                            if(t !== true) {
                                fullClose = false;
                            }
                        })
                ...
                if(Object.keys(modConn).length === 0) {
            ...
            me.action.forEach(function(action, i) {
                ...
                setModules.forEach(function(o) {
                    ...
                    var onReady = function() {
                        ...
                        // モジュール内部エラーハンドラ
                        var modError = function(err) {
                            mod.removeListener('error', onReady);
                            me.emit('error', err);
                        };
                        mod.on('error', modError);

                        action.emit('bindmodule', name, mod);
                    };
                    */

                    mod.once('ready', function() {
                        action.emit('bindmodule', name, mod);
                    });
