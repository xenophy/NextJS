...
NX.controller.AbstractController = NX.extend(NX.util.Observable, {
    ...
    createModuleObjects : function(use) {
        ...
        NX.each(use, function(name) {
            ...
            NX.iterate(NX.Module.prototype, function(name) {
                ignoreMembers[name] = true;
            })

            userMethods[modName] = [];

            NX.iterate(moduleCls.prototype, function(name, member) {
                if(ignoreMembers[name] !== true && NX.isFunction(member)) {
                    userMethods[modName].push(name);
                }
            });
