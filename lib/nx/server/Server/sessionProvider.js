...
module.exports = function(config) {
    ...
    if(NX.isString(config.type)) {

        var stype = NX.String.format(
            'NX.data.{0}Store',
            NX.String.capitalize(config.type.toLowerCase())
        );

        config.store = NX.create(stype, config);

    } else {

        // Storeタイプが指定されていない場合は、FileStoreを生成
        config.store = NX.create('NX.data.FileStore', {
            savepath: config.savepath || '/tmp'
        });
